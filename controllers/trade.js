...
//         {model: db.user, through: {where: {trade_copies: {[op.gt]:0}}}, where: {[op.not]: {id: req.user.id}}},
//         {model: db.set}
//       ]}
//     ]
//   }).then(result => {
...
//         let userCollection = {
//           "card.cardPrintings.set.code": result.cardPrintings[card].set.code,
//           "card.cardPrintings.set.title": result.cardPrintings[card].set.title,
//           "card.cardPrintings.id": result.cardPrintings[card].id,
//           "card.name": result.name,
//           "card.cardPrintings.users.collection.foil": currentUser.collection.foil,
//           "card.cardPrintings.users.collection.owned_copies": currentUser.collection.owned_copies,
//           "card.cardPrintings.users.collection.trade_copies": currentUser.collection.trade_copies
//         }
...
//     }, include: [
//       {model: db.card, where: {name: req.params.name}, include: [
//         {model: db.printings, as: 'cardPrintings'}
//       ]},
//       {model: db.user, required: true, attributes: {exclude: ['password','createdAt','updatedAt']}}
//     ]
//   }).then(result => {
...
//       name: req.params.name
//     }, include: [
//       {model: db.printings, as: 'cardPrintings', limit: 1, include: [
//         {model: db.set}
//       ]},
//       // {model: db.wishlist, include: [
//       //   {model: db.user, required: true, where: {[op.not]: req.user.id}}
//       // ]}
...
//     //   let currentUser = result.wishlists[card].user.dataValues
//     //   let userWishlist = {
//     //     "printing.id": result.wishlists[card].card.cardPrintings[0].id,
//     //     "printing.card.name": x,
//     //     "printing.card.users.wishlist.number_wanted": x,
//     //     "printing.card.users.wishlist.pref_printing": x,
//     //     "printing.set.title": x,
//     //     "printing.card.users.wishlist.pref_foil": x,
//     //   }
//     //   if (providePartners.hasOwnProperty(currentUser.id)) {
//     //     providePartners[currentUser.id].cards.push(result.wishlists[card])
//     //   } else {
//     //     providePartners[currentUser.id] = {
//     //       username: currentUser.username,
//     //       userId: currentUser.id,
//     //       cards: [result.wishlists[card]]
//     //     }
//     //   }
//     // }
//     res.json(result);
...
//     res.json({
//       error: true,
//       message: "Could not find card for PROVIDE"
//     });
//   })
// })
...
//       {model: db.wishlist, include: [
//         {model: db.user, required: true, where: {[op.not]: {id: req.user.id}}},
//         {model: db.card, include: [{model: db.printings, as: 'cardPrintings', duplicating: false, limit: 1}]}
//       ]}
//     ]
//   }).then(result => {
//     // Handle Acquire Partners

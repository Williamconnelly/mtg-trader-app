...
router.get("/acquire", verifyToken, (req, res) => {
...
router.get("/provide", verifyToken, (req, res) => {
...
router.get("/acquire/card/:name", verifyToken, (req, res) => {
  db.collection.findAll({
    raw: true,
    attributes: ['userId','owned_copies','trade_copies','foil','card_condition'],
    where: {
...
router.get("/provide/card/:name", verifyToken, (req, res) => {
  db.wishlist.findAll({
...
router.get("/acquire/user/:name", verifyToken, (req, res) => {
  db.collection.findAll({
    attributes: ['userId','owned_copies','trade_copies','foil','card_condition'],
    where: {
      trade_copies: {[op.gt]:0}
...
router.get("/provide/user/:name", verifyToken, (req, res) => {
  db.wishlist.findAll({
    attributes: {exclude: ['createdAt','updatedAt']},
    include: [
